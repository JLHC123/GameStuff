...
def drawWinScreen(screen, selected_option):
    # system font for win screen
    font = pygame.font.SysFont(None, 20)
    text1 = font.render("You Win!", True, (255, 255, 0)) # yellow
    screen.blit(text1, (0, 0))
    
    options = ["Play Again?", "Exit Game"]
    for i, option in enumerate(options):
        # if selected option, highlight it red, else don't (yellow)
        color = (255, 0, 0) if i == selected_option else (255, 255, 0)
        # and add indicator for selected option
        text = font.render(option + (" <" if i == selected_option else ""), True, color)
        screen.blit(text, (0, 20 + i * 20)) # (0, 40) and (0, 60)

def playingMaze(screen, n):
    # make a random maze
    ...
    while running:
            ...
            # to prevent blitz movement we only move one tile per key press
                ...
                if game_state == PLAYING:
                    ...
                    if event.key == pygame.K_UP:
                        ...
                        # this is needed to return to main game loop (without it the game freezes at the win screen)
        ...
        # display win screen
            ...
            drawWinScreen(screen, selected_option)
...
def main():
    ...
    # main game loop
    while True:
        result = playingMaze(screen, n)
        if result == "quit":
            break
        # else we continue playing the maze by making a new one
    pygame.quit()
